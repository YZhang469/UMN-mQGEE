...
Q <- function(dat, method = "SQ"){ # method can have value "SQ" or "MQ"
  ...
  if (method == "MQ"){
    # modified Q-learning uses the observed values and adds back the reward if the observed treatment is suboptimal
...
## Q-learning methods with GEE

QGEE <- function(dat, method = "SQGEE", corstr = c("unstructured", "exchangeable")){ # method can be "SQGEE" or "MQGEE"
  n <- nrow(dat)
  
  ## stage 2
  dat.s2 <- cbind.data.frame(do.call(rbind, replicate(2, dat[ , c("ID", "Y0", "A1", "Y1", "A2", "Y2", "Y3")], simplify = FALSE)), 
                             "time" = rep(c(2, 3), each = n), "Q2" = c(dat$Y2, dat$Y3))
  ...
  dat.s2.opt <- dat.s2 # under optimal treatment at stage 2: dat.s2.opt
  # A2 = -1
  dat.s2a <- dat.s2
  dat.s2a$A2 <- -1
  dat.s2a$Q2 <- predict(mod.s2, newdata = dat.s2a)
  ...
  dat.s1.opt <- dat.s1 # under optimal treatment at stage 1: dat.s1.opt
  # A2 = -1
  dat.s1a <- dat.s1
  dat.s1a$A1 <- -1
  dat.s1a$Q1 <- predict(mod.s1, newdata = dat.s1a)
